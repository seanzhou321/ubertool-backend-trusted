...
**Date:** May 11, 2026  
**Endpoint:** `api.marigoldshare.marigoldintelligence.us:50052`  
...
| Certificate | Let's Encrypt — issued to `api.marigoldshare.marigoldintelligence.us` |
...
The client MUST connect to the hostname `api.marigoldshare.marigoldintelligence.us` (not the raw IP `35.160.176.235`). The certificate's CN/SAN is bound to the hostname. Connecting by IP will fail hostname verification.
...
```
Android Client                          EC2 Server (api.marigoldshare.marigoldintelligence.us:50052)
     │                                            │
     │──── TCP SYN ──────────────────────────────▶│
     │◀─── TCP SYN-ACK ──────────────────────────│
     │                                            │
     ...
     │     (api.marigoldshare.marigoldintelligence.us, Let's Encrypt)    │
     │                                            │
     │  [Client validates cert:]                  │
     │  - Chain → ISRG Root X1 (system CA store)  │
     │  - CN/SAN matches api.marigoldshare.marigoldintelligence.us       │
     │  - Cert not expired                        │
     │                                            │
...
| `SSLPeerUnverifiedException: Hostname not verified` | Client connected to raw IP `35.160.176.235` instead of `api.marigoldshare.marigoldintelligence.us` |
...
The server-side smoke test `TestTLSConnectivity` (in `tests/smoke/`) performs a raw TLS dial to `api.marigoldshare.marigoldintelligence.us:50052` and verifies:
