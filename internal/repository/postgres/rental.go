...
func (r *rentalRepository) GetByID(ctx context.Context, id int32) (*domain.Rental, error) {
	rt := &domain.Rental{}
	query := `SELECT id, org_id, tool_id, renter_id, owner_id, start_date, scheduled_end_date, end_date, total_cost_cents, status, pickup_note, created_on, updated_on FROM rentals WHERE id = $1`
	err := r.db.QueryRowContext(ctx, query, id).Scan(&rt.ID, &rt.OrgID, &rt.ToolID, &rt.RenterID, &rt.OwnerID, &rt.StartDate, &rt.ScheduledEndDate, &rt.EndDate, &rt.TotalCostCents, &rt.Status, &rt.PickupNote, &rt.CreatedOn, &rt.UpdatedOn)
	if err != nil {
		return nil, err
	}
	return rt, nil
...
func (r *rentalRepository) Update(ctx context.Context, rt *domain.Rental) error {
	query := `UPDATE rentals SET status=$1, pickup_note=$2, end_date=$3, updated_on=$4 WHERE id=$5`
	_, err := r.db.ExecContext(ctx, query, rt.Status, rt.PickupNote, rt.EndDate, time.Now(), rt.ID)
	return err
}
...
func (r *rentalRepository) ListByRenter(ctx context.Context, renterID, orgID int32, status string, page, pageSize int32) ([]domain.Rental, int32, error) {
	...
	sql := `SELECT id, org_id, tool_id, renter_id, owner_id, start_date, scheduled_end_date, end_date, total_cost_cents, status, pickup_note, created_on, updated_on 
	...
	for rows.Next() {
		...
		if err := rows.Scan(&rt.ID, &rt.OrgID, &rt.ToolID, &rt.RenterID, &rt.OwnerID, &rt.StartDate, &rt.ScheduledEndDate, &rt.EndDate, &rt.TotalCostCents, &rt.Status, &rt.PickupNote, &rt.CreatedOn, &rt.UpdatedOn); err != nil {
			return nil, 0, err
		}
