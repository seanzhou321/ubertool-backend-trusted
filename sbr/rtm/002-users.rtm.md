...
|---|---|---|---|---|---|---|---|
| FR-001 | `GetUser` MUST return the caller's own profile (identity from JWT) joined with every org they belong to, including per-org role and balance. | `TestUserService_GetUserProfile > "Returns each org membership with its role and balance intact"` (`tests/unit/user_service_test.go`) | **Intentionally not separately tested** — see Notes | `TestUserService_E2E > "GetUser Profile"` (`tests/e2e/user_test.go:23`) — now also asserts `org.UserRole` (`MEMBER`/`ADMIN`) per org, not just `UserBalance` | `TestSmoke... > "UsersTableReachable_via_Login"` (`tests/smoke/smoke_test.go:142`) | **Complete** | **Fixed 2026-07-23 (SBR remediation Phase 3).** The e2e gap (role field previously unasserted) and the total absence of a unit test for `userService.GetUserProfile` are both closed. No bug found — production behavior matched the spec exactly. L2 not added (2026-07-23 hardening pass): the e2e test already calls `GetUser` through the real gRPC/DB stack and asserts the exact per-org role/balance join this FR names, directly against Postgres — L3 subsumes L2 here. |
| FR-002 | `GetUser` MUST NOT fail outright when one membership's org record can't be loaded — MUST return the remaining memberships. | `TestUserService_GetUserProfile > "Skips a membership whose org record can't be loaded, returning the rest"` (`tests/unit/user_service_test.go`) | **N/A** — see Notes | **N/A** — see Notes | — | **Complete** | **Fixed 2026-07-23 (SBR remediation Phase 3).** Confirms a single failed org lookup among 3 memberships is skipped, not fatal — the other 2 orgs are still returned. No bug found. **L2/L3 not added (2026-07-23 hardening pass):** confirmed by reading the schema (`podman/trusted-group/postgres/ubertool_schema_trusted.sql`) that `users_orgs.org_id REFERENCES orgs(id) ON DELETE CASCADE` — a real Postgres row can never reference a nonexistent org, so the "org record can't be loaded" scenario this FR names is structurally unreachable against a real database. A mocked `orgRepo.GetByID` error (as L1 does) is the only tier that can actually exercise this clause; an L2/L3 test would need to break referential integrity out-of-band to even attempt it, which wouldn't represent a reachable production state. |
...
| FR-005 | As-built, `UpdateProfile` MUST NOT be assumed to validate email format or reject empty `name`/`phone`/`email` beyond DB `NOT NULL` constraints. | **N/A** — see Notes | **Intentionally not separately tested** — see Notes | `TestUserService_E2E > "UpdateProfile accepts malformed/empty input (documents absence of validation)"` (`tests/e2e/user_test.go`) — submits an empty `name`, empty `phone`, and a malformed (non-`@`) `email`, asserts the call succeeds and the DB stores exactly those values | — | **Complete** | **Fixed 2026-07-23 (SBR remediation Phase 4).** This is a regression-lock on the as-built lenient (non-)validation behavior, not a design fix — if input validation is ever added, this test will fail, correctly signaling the documented behavior changed on purpose. No bug found (behavior confirmed exactly as spec.md's Coverage Baseline described). **L1/L2 not added (2026-07-23 hardening pass):** this is a negative/absence claim about real DB-level constraint behavior — a mocked repo (L1) would only reflect the mock's own permissiveness, telling us nothing about whether Postgres itself would reject malformed input, so it can't provide meaningful evidence either way. Confirmed by reading `UserHandler.UpdateProfile` (`internal/api/grpc/user.go:53`) that the handler adds no validation of its own before calling the service, so the e2e test's full-stack, real-DB assertion is the only tier that actually proves this claim — an L2 test would exercise the identical repo call with no additional handler-layer behavior to differentiate, making it redundant. |
...
- **Phase 4 outcome**: FR-004's email-uniqueness rejection closed at L1 (service propagates the
  ...
  either.
- **Phase 4 hardening pass (2026-07-23)**: re-audited every cell for unexplained blanks per
  `sbr/README.md`'s rule that every omitted tier must be justified. No additional tests were
  warranted here (unlike 001's hardening pass) — every remaining blank turned out to be a
  legitimate omission once investigated: FR-001's L2 is subsumed by its e2e evidence; FR-002's
  L2/L3 are structurally unreachable (`users_orgs.org_id` has an `ON DELETE CASCADE` FK to
  `orgs`, so a real DB row can never reference a missing org — only a mocked repo can exercise
  this clause); FR-005's L1/L2 can't meaningfully test a negative real-DB-behavior claim (a
  mocked repo reflects only its own permissiveness, and the handler adds no logic beyond what
  the e2e test already proves against a real DB). All rows updated with explicit Notes
  justification; no new gaps found.
