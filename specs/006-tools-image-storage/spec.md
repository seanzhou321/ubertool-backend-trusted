...
1. **Given** an authenticated user, **When** `AddTool` is called, **Then** a `tools` row is
   ...
   (`file_name`/`file_path`/`thumbnail_path` all set to the raw URL — see Edge Cases).
...
3. **Given** the caller is the tool's owner, **When** `UpdateTool` is called, **Then** the
   ...
   metro, duration) are overwritten. This requires the caller's ID to match
   `tool.OwnerID`, fetched fresh from the database before the write.
...
repository CRUD (integration); `UpdateTool`/`DeleteTool` ownership rejection and acceptance,
both RPCs, via `TestToolService_UpdateDelete_RequiresOwnership` (unit);
`ConfirmImageUpload`/`DeleteImage`/`SetPrimaryImage`'s ownership-rejection clauses (FR-004)
and `GetDownloadUrl`'s owner-or-`AVAILABLE` disjunctive boundary (FR-005), via
...
- `SetPrimaryImage`'s validation rejection paths (image belongs to a different tool; image
  not yet `CONFIRMED`) — its ownership rejection is covered (see above), these two are not.
...
- **FR-001**: `UpdateTool` and `DeleteTool` MUST require the caller to be the tool's
  `owner_id`, verified by a fresh database read before any write.
...
- `docs/design/image-storage/` design artifacts were consulted for cross-reference but not
  ...
  parameter names); a deeper pass is reasonable future follow-up if this domain is
  revisited.
- Whether `GetToolImages` should require the `GetDownloadUrl`-style access check (Known
  Discrepancy 1) is a product decision this spec documents but does not prescribe.
