...
func TestRentalRepository_GetByID(t *testing.T) {
	...
	t.Run("Success", func(t *testing.T) {
		rows := sqlmock.NewRows([]string{"id", "org_id", "tool_id", "renter_id", "owner_id", "start_date", "scheduled_end_date", "end_date", "total_cost_cents", "status", "pickup_note", "created_on", "updated_on"}).
			AddRow(1, 1, 2, 3, 4, time.Now(), time.Now(), nil, 1000, "PENDING", "Note", time.Now(), time.Now())
